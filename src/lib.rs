...
fn find_executable(executable_name: &str) -> Option<String> {
    if let Ok(path) = env::var("PATH") {
        for dir in path.split(':') {
            let full_path = format!("{}/{}", dir, executable_name);
            if Path::new(&full_path).exists() {
                return Some(full_path);
            }
        }
    }
    eprintln!("Executable '{}' not found in PATH.", executable_name);
    None
}
...
fn index_bam(bam_file: &str, samtools_path: &str) -> io::Result<()> {
...
fn get_coverage(bam_file: &str, chromosomes: &[&str], temp_output_file: &str, activate_path: &str) -> io::Result<()> {
    // 执行 bedtools genomecov 命令以计算覆盖度，结果保存到临时文件
    let status = Command::new("sh")
        .arg("-c")
        .arg(format!(
            "source {} rust && bedtools genomecov -ibam {} -bg > {}",
            activate_path,
            bam_file,
            temp_output_file
        ))
...
pub fn bam_merge(
    input_name: &str, 
    file_list_path: Option<&str>, // 变为可选参数
    bam_dir: &str, 
    work_dir: &str, 
    chromosome_file: &str, 
    chromosomes: &[&str]
    ...
    // 查找工具路径
    let samtools_path = find_executable("samtools")
        .unwrap_or_else(|| {
            let default_path = "/data/haocheng/miniconda3/envs/rust/bin/samtools".to_string();
            println!("samtools not found in PATH. Using default path: {}", default_path);
            default_path
        });

    let activate_path = find_executable("bedtools")
        .unwrap_or_else(|| {
            let default_path = "/data/haocheng/miniconda3/bin/bedtools".to_string();
            println!("bedtools not found in PATH. Using default path: {}", default_path);
            default_path
        });

    // 执行下载步骤，如果提供了 file_list_path
    if let Some(path) = file_list_path {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(async {
            download_files_from_list(path, bam_dir).await
        })?;
    }
    ...
    for bam_file in &bam_files {
        index_bam(bam_file, &samtools_path)?;
    }
    ...
    if is_file_empty(&output_file1).unwrap_or(false) {
        get_coverage(&first_bam, chromosomes, &temp_output_file1, &activate_path)?;
        filter_chromosome_data(&temp_output_file1, chromosomes, &output_file1)?;
    } else {
