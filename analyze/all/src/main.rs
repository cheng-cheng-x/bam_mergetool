...
pub fn process_files(input_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    //let chunk_size = 1000;
    //let chromosome_data1 = process_file_in_chunks(&output_file1_clone, chunk_size)
    //.with_context(|| format!("Error processing file {}", &output_file1_clone))?;
/* 
...
let handle1 = thread::spawn(move || {
    if is_file_empty(&output_file1_clone).unwrap_or(false) {
        get_coverage(&first_bam_clone, &chromosomes_clone1, &temp_output_file1_clone).unwrap();
        filter_chromosome_data(&temp_output_file1_clone, &chromosomes_clone1, &output_file1_clone).unwrap();
    } else {
        println!("{} is not empty. Skipping coverage calculation.", output_file1_clone);
    }
    let chromosome_data1 = build_chromosome_data(&output_file1_clone).unwrap();
    println!("Processed {} with data from {}", chromosome_data1.len(), output_file1_clone);
    tx1.send(chromosome_data1).unwrap();
});

let tx2 = tx.clone();
let handle2 = thread::spawn(move || {
    if is_file_empty(&output_file2_clone).unwrap_or(false) {
        get_coverage(&second_bam_clone, &chromosomes_clone2, &temp_output_file2_clone).unwrap();
        filter_chromosome_data(&temp_output_file2_clone, &chromosomes_clone2, &output_file2_clone).unwrap();
    } else {
        println!("{} is not empty. Skipping coverage calculation.", output_file2_clone);
    }
    let chromosome_data2 = build_chromosome_data(&output_file2_clone).unwrap();
    println!("Processed {} with data from {}", chromosome_data2.len(), output_file2_clone);
    tx2.send(chromosome_data2).unwrap();
});
...
// 统一接收两个线程的结果
let chromosome_data1 = rx.recv().unwrap();
let chromosome_data2 = rx.recv().unwrap();
