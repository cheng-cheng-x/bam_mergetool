...
# 定义要执行的命令
command = "aria2c -s 8 -x 16 -i /data/haocheng/data/ACAT/txt/A549.txt -d /data/haocheng/data/bam/A549"
# 使用 subprocess 运行命令
try:
...
# 设置BAM文件目录
bam_dir = "/data/haocheng/data/bam/A549/"
...
# 源文件路径
src_file = "/data/haocheng/data/bam/A549/sample1_signal.bed"
# 目标文件路径
dest_file = "/data/haocheng/data/bam/result/A549.bed"
...
# 设置要删除文件的目录
directory = "/data/haocheng/data/bam/A549"

# 遍历目录
for filename in os.listdir(directory):
    # 检查文件扩展名是否为.bam或.bam.bai
    if filename.endswith(".bam") or filename.endswith(".bqiuam.bai"):
        # 构建完整的文件路径
...
# 定义要执行的命令
command = "aria2c -s 8 -x 16 -i /data/haocheng/data/ACAT/txt/HepG2.txt -d /data/haocheng/data/bam/HepG2"
# 使用 subprocess 运行命令
try:
    subprocess.run(command, shell=True, check=True)
    print("Download completed successfully.")
...
# 设置BAM文件目录
bam_dir = "/data/haocheng/data/bam/HepG2/"
bam_files = sorted([f for f in os.listdir(bam_dir) if f.endswith('.bam')])
# 获取第一个和第二个BAM文件的路径
bam_file1 = os.path.join(bam_dir, bam_files[0])
bam_file2 = os.path.join(bam_dir, bam_files[1])
...
# 源文件路径
src_file = "/data/haocheng/data/bam/HepG2/sample1_signal.bed"
# 目标文件路径
dest_file = "/data/haocheng/data/bam/result/HepG2.bed"
...
# 设置要删除文件的目录
directory = "/data/haocheng/data/bam/HepG2"
